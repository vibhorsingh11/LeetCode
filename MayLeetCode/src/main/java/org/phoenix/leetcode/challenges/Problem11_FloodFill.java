...
public class Problem11_FloodFill {
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        // determining the starting pixel color
        int color = image[sr][sc];
        // if start pixel color is not equal to new color then call the function
        if (color != newColor) dfs(image, sr, sc, color, newColor);
        return image;
    }

    public void dfs(int[][] image, int r, int c, int color, int newColor) {
        // execute code only if pixel has initial pixel color
        if (image[r][c] == color) {
            // change the current pixel color
            image[r][c] = newColor;
            // handling base cases and filling colors in 4-direction
