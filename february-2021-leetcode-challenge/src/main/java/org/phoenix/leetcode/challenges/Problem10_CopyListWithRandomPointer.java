...
public class Problem10_CopyListWithRandomPointer {
    public Node copyRandomList(Node head) {
        ...
        // creating reference to curr node
        Node curr = head;
        // creating reference to clone node
        Node clone;
        // Map to store original and cloned node
        Map<Node, Node> map = new HashMap<>();
        // looping over current node and adding new node to map
        while (curr != null) {
        ...
        // reassigning curr
        curr = head;
        // looping again over current node and cloning them
        while (curr != null) {
            clone = map.get(curr);
